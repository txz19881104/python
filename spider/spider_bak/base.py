...
class Comics(EntertainmentSpider):
        ...
        for url in dct_img_book['download_url']:
            ...
            for i in range(5):
                if not BaseRequest.DownloadData(url, path, file_name):
                    ...
                    #print("download %s%s success" % (path,file_name))
    ...
    def _UpdateImg(self, dct_img_book):
        ...
        for url in dct_img_book['download_url']:
            ...
            Img_src  = "\"" + url + "\""
            for i in range(10):
                sql = "update EntertainmentDB.ComicImgSrc set Img_src = %s  where Comic_ChapterNum = %d and Comic_ID = %d and Page_Num = %d;" %(Img_src, chapter_num, ID, count)
...
class Fiction(EntertainmentSpider):
    ...
    def _InsertFiction(self, chapter_num, dct_img_book, download_path=None):
        ...
        sql_dict['ChapterNum']   = chapter_num              #章节号
        sql_dict['ChapterName']  = "\"" + title + "\""      #章节名称
        sql_dict['ContentUrl']   = "\"" + "https://txz-1256783950.cos.ap-beijing.myqcloud.com/Fiction/" + self.keyword + "/"  + file_name + "\""      #小说内容
        sql_dict['Dept_ID']      = ID                       #外键，关联漫画名称表，与其id相同
    ...
    def _UpdateFiction(self, dct_img_book):
        """更新漫画图片
