...
#打开num个线程
def StartComicThread(num):
    for i in range(num):  
        dt = DatamineThreadComic(priority_queue)#线程任务就是从源代码中解析出<title>标签内的内容  
        #dt.setDaemon(True)  
        dt.start() 
        threads.append(dt)

#漫画
class DatamineThreadComic(threading.Thread):  
    ...
    def run(self): 
        ...
        while IsHaveData:
            ...
            try:   
                chunk = self.queue.get(block=True, timeout=120) 
            except queue.Empty:
                IsHaveData = False
                continue
