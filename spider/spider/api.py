...
from Comics import ComicProcesses
...
class EntertainmentAPI(object):
    ...
    def GetComicByKeyword(self):
        ...
        """
        download_lst = [
            {"name":'西行纪', "url": "http://www.kuaikanmanhua.com", "download": "/mnt/TecentCloud"},
        ]
        print(download_lst)
    ...
    def UpdateComicChapter(self, download_path=None):
        ...
        for result in results_tup:
            ...
            if chapter_num_max > result[4]:
                sql = "update EntertainmentDB.tbl_comic_name set chapter_count = %d  where pk_id = %d;" %(chapter_num, comic_id)
    ...
    def UpdateFictionChapter(self, download_path=None):
        ...
        #查找所有未完结的漫画
        ComicHandle = EntertainmentSpider()
        sql = "SELECT * FROM EntertainmentDB.tbl_fiction_name WHERE is_finish = 0;"
        results_tup = ComicHandle._EntertainmentSelect(sql)
        ...
        for result in results_tup:
            ...
            #更新当前数据库里的最大章节数
            if chapter_num_max > result[4]:
                sql = "update EntertainmentDB.tbl_fiction_name set chapter_count = %d  where pk_id = %d;" %(chapter_num_max, fiction_id)
                print(sql)
        ...
        for count in range(num+1):
            ...
            if not priority_queue.empty():
                ...
                if (count % 115 == 0 or count == num) and count != 0:
                    ...
                    process = multiprocessing.Process(target=ComicProcesses,args=(task,))
                    process.start()
                    size = 0
                    task = []
