...
class FictionBQG(Fiction):
    ...
    def _GetContentByKeyword(self, keyword, mode, download_path=None):
        ...
        for result in find_result:
            if mode == "download":
                #判断此漫画是否已经下载过
                sql = "SELECT * FROM EntertainmentDB.FictionName WHERE Name=\"%s\";" %(result["name"])
                ...
                if not self._EntertainmentInsert('FictionName', sql_dict):
                    print("inster FictionName table failed!")
                    continue


                #获取漫画编号，唯一
                sql = "SELECT ID FROM EntertainmentDB.FictionName WHERE Name=\"%s\";" %(a_name['content'])
                ...
                sql = "update EntertainmentDB.FictionName set Time = %s  where ID = %d;" %(now_Time, self.id)
